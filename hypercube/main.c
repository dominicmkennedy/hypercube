...
void set(char *d, COORD pt, char c) { d[pt.Y * ww + pt.X] = c; }
...
void ln(char *d, COORD a, COORD b) {
  set(d, a, '@');
  set(d, b, '@');
...
double norm4(const double *V) { return sqrt(dot4(V, V)); }
...
void projectTo3D(double vAngle, const double *matView,
                 const double *matRotation) {
  // column vectors
...
double norm3(const double *V) { return sqrt(dot3(V, V)); }
...
void projectTo2D(double vAngle, const double *matView,
                 const double *matRotation) {
  // column vectors
